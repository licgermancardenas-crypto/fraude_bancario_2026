...
### Insight 9 — El GNN mejora 0.103 puntos de PR-AUC sobre XGBoost

> **Hallazgo:** GraphSAGE logra PR-AUC=0.946 vs XGBoost=0.843 vs LogReg=0.646, usando exactamente el mismo test set y splits. El único diferencial es el acceso a la estructura del grafo.
...
### Insight 10 — La topología sola (ablation) logra PR-AUC=0.018

> **Hallazgo:** Un GraphSAGE entrenado con features constantes (solo la estructura del grafo, sin balance, risk_score ni ningún atributo de cuenta) alcanza PR-AUC=0.018. La información de *quién se conecta con quién* ya contiene señal de fraude independiente de los features individuales.
...
> **Hallazgo:** El score mínimo entre nodos fraudulentos es 0.0013. Los nodos con score más bajo tienden a tener menor grado — son los nodos "de entrada" o "de salida" del anillo que tienen menos vecinos fraudulentos directos y por lo tanto reciben menos señal de red.
...
> **Hallazgo:** LogReg logra ROC-AUC=0.969 (alta separabilidad global) pero PR-AUC=0.646 (baja precisión en clases desbalanceadas) y Recall@P90=0. La frontera de decisión lineal no puede separar fraude de legítimo en el espacio de features en el punto operativo (90% precisión).
