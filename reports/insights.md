...
### Insight 3 — Homofilia 7.3x: los vecinos del fraude tienden fuertemente a ser fraude

> **Hallazgo:** Las aristas fraude→fraude representan el 0.593% del grafo, vs. un 0.081% esperado bajo distribución aleatoria — un lift de **7.3x**. El coeficiente de homofilia global es h = 0.952. (El lift bajó respecto a corridas previas al incorporar la economía legítima de empresas/comercios, que añade aristas legítimas de alto volumen que diluyen la fracción fraude→fraude.)

> **Implicancia para BRS:** La señal de fraude no está en la cuenta individual sino en su vecindario inmediato: una cuenta conectada a fraude tiene ~7x más probabilidad de ser fraude que el azar. Un modelo que ignore la estructura del grafo (como Regresión Logística o XGBoost tabulares) descarta ese lift que existe en las conexiones.
...
**Hallazgo:** GNNExplainer revela que los features más determinantes para clasificar una cuenta como fraude son **degree_out**, **balance_retention** y **unique_receivers** — todos indicadores de conectividad y comportamiento relacional, no de monto absoluto.
...
### Insight 14 — El 12% de los vecinos influyentes son también de alto riesgo

**Hallazgo:** Para los top-5 nodos de fraude explicados, el 12% de sus vecinos más influyentes (edge mask > 25% del máximo) tienen a su vez un score GNN > 0.5. La señal de fraude se propaga de forma consistente a través de la red.
...
### Insight 9 — El GNN mejora 0.023 puntos de PR-AUC sobre XGBoost

> **Hallazgo:** GraphSAGE logra PR-AUC=0.991 vs XGBoost=0.968 vs LogReg=0.844, usando exactamente el mismo test set y splits. El único diferencial es el acceso a la estructura del grafo.
...
### Insight 10 — La topología sola (ablation) logra PR-AUC=0.037

> **Hallazgo:** Un GraphSAGE entrenado con features constantes (solo la estructura del grafo, sin balance, risk_score ni ningún atributo de cuenta) alcanza PR-AUC=0.037. La información de *quién se conecta con quién* ya contiene señal de fraude independiente de los features individuales.
...
> **Hallazgo:** El score mínimo entre nodos fraudulentos es 0.1446. Los nodos con score más bajo tienden a tener menor grado — son los nodos "de entrada" o "de salida" del anillo que tienen menos vecinos fraudulentos directos y por lo tanto reciben menos señal de red.
...
> **Hallazgo:** LogReg logra ROC-AUC=0.990 (alta separabilidad global) pero PR-AUC=0.844 (baja precisión en clases desbalanceadas) y Recall@P90=0. La frontera de decisión lineal no puede separar fraude de legítimo en el espacio de features en el punto operativo (90% precisión).
...
**Hallazgo:** GNNExplainer revela que los features más determinantes para clasificar una cuenta como fraude son **type_merchant**, **unique_receivers** y **mean_interval_hours** — todos indicadores de conectividad y comportamiento relacional, no de monto absoluto.
...
### Insight 9 — El GNN mejora 0.028 puntos de PR-AUC sobre XGBoost

> **Hallazgo:** GraphSAGE logra PR-AUC=0.978 vs XGBoost=0.950 vs LogReg=0.728, usando exactamente el mismo test set y splits. El único diferencial es el acceso a la estructura del grafo.
...
### Insight 10 — La topología sola (ablation) logra PR-AUC=0.029

> **Hallazgo:** Un GraphSAGE entrenado con features constantes (solo la estructura del grafo, sin balance, risk_score ni ningún atributo de cuenta) alcanza PR-AUC=0.029. La información de *quién se conecta con quién* ya contiene señal de fraude independiente de los features individuales.
...
> **Hallazgo:** El score mínimo entre nodos fraudulentos es 0.1181. Los nodos con score más bajo tienden a tener menor grado — son los nodos "de entrada" o "de salida" del anillo que tienen menos vecinos fraudulentos directos y por lo tanto reciben menos señal de red.
...
> **Hallazgo:** LogReg logra ROC-AUC=0.976 (alta separabilidad global) pero PR-AUC=0.728 (baja precisión en clases desbalanceadas) y Recall@P90=0. La frontera de decisión lineal no puede separar fraude de legítimo en el espacio de features en el punto operativo (90% precisión).
