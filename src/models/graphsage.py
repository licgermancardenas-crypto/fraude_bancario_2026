"""
GraphSAGE node classification model for fraud detection.

Architecture (per project spec):
  SAGEConv(in → 64) → ReLU → Dropout(0.3)
  SAGEConv(64 → 64) → ReLU → Dropout(0.3)
  Linear(64 → 2)

Why GraphSAGE over GCN/GAT:
  - Inductive: generalises to new accounts without retraining (production argument)
  - Scales with neighbour sampling (needed when moving to full BRS data)
  - Strong empirical baseline on heterophilic fraud graphs
"""

import torch
import torch.nn as nn
import torch.nn.functional as F
from torch_geometric.nn import SAGEConv


class GraphSAGE(nn.Module):
    def __init__(self, in_channels: int, hidden_channels: int = 64,
                 num_layers: int = 2, dropout: float = 0.3):
        super().__init__()
        self.dropout = dropout

        self.convs = nn.ModuleList()
        self.convs.append(SAGEConv(in_channels, hidden_channels))
        for _ in range(num_layers - 1):
            self.convs.append(SAGEConv(hidden_channels, hidden_channels))

        self.classifier = nn.Linear(hidden_channels, 2)

    def forward(self, x: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        for conv in self.convs:
            x = conv(x, edge_index)
            x = F.relu(x)
            x = F.dropout(x, p=self.dropout, training=self.training)
        return self.classifier(x)

    def predict_proba(self, x: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        """Return probability of class 1 (fraud)."""
        logits = self.forward(x, edge_index)
        return F.softmax(logits, dim=1)[:, 1]
